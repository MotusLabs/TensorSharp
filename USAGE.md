...
| `-hf <org>/<repo>[:<quant>]` / `--hf-repo <org>/<repo>[:<quant>]` | Resolve the model from the local Hugging Face hub cache by repo id instead of a path — e.g. `-hf unsloth/Qwen3.8-27B-GGUF:UD-IQ3_XXS`. Mutually exclusive with `--model`; nothing is ever downloaded, and a repo or quant that is not cached fails with the `hf download <repo>` command to run first (which is also what puts it in the cache). Without a quant tag: `Q4_K_M`, then `Q8_0`, then the first model GGUF; split GGUFs pick the `-00001-of-` shard and the remaining shards load from it automatically. The cache root follows the documented client precedence: `HF_HUB_CACHE` → `HUGGINGFACE_HUB_CACHE` → `HF_HOME/hub` → `XDG_CACHE_HOME/huggingface/hub` → `~/.cache/huggingface/hub` |
| `--hf-file <path>` | Exact repo-relative file inside the `-hf` / `--hf-repo` cache repo (e.g. `UD-Q2_K_XL/model-00001-of-00003.gguf`), overriding quant matching |
...
| `--mmproj <path>` | Path to the multimodal projector GGUF file. Also accepts an `<org>/<repo>[:<quant>]` Hugging Face cache id; omitted beside `-hf`, a projector cached in the same repo is picked up automatically (`--mmproj none` disables that). |
...
| `-hf <org>/<repo>[:<quant>]` / `--hf-repo <org>/<repo>[:<quant>]` | Resolve the hosted model from the local Hugging Face hub cache by repo id instead of a path — e.g. `-hf unsloth/Qwen3.8-27B-GGUF:UD-IQ3_XXS`. Cache-only: nothing is downloaded, and a repo or quant that is not cached fails startup with the `hf download <repo>` command to run first. Quant fallback and cache-root discovery as under the CLI's flag of the same name; mutually exclusive with `--model` |
| `--hf-file <path>` | Exact repo-relative file inside the `-hf` / `--hf-repo` cache repo, overriding quant matching |
| `--mmproj <path>` | Multimodal projector GGUF (resolved relative to the model directory when only a filename is given; pass `none` to disable). Also accepts an `<org>/<repo>[:<quant>]` cache id, and beside `-hf` a projector cached in the same repo is picked up automatically when no projector was named. Requires `--model` or `-hf`. |
